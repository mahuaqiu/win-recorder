...
impl MFSinkWriter {
    ...
    /// 输入类型为 MFVideoFormat_RGB32 (BGRA)，输出类型为 MFVideoFormat_H264
    /// 分辨率会自动对齐到 16 倍数（H264 编码器要求）
    /// 编码数据写入 byte_stream 而非文件，用于内存输出
    pub fn from_byte_stream(
        ...
        unsafe {
            ...
            // 使用 MFCreateSinkWriterFromURL 创建 SinkWriter，传入 byte_stream 实现内存输出
            let sink_writer = MFCreateSinkWriterFromURL(
                PCWSTR::null(),
                Some(&byte_stream),
                None::<&IMFAttributes>,
            )
            .map_err(|e| RecorderError::MFError(format!("创建 SinkWriter (ByteStream) 失败: {}", e)))?;
